...
## Week Reflection
Week 2 felt different from Week 1 because it became less about just getting the agent working and more about understanding what was happening under the hood. After rebuilding the Khopfa Towing agent on the Anthropic SDK, I noticed that the overall architecture stayed the same — messages, tools, loops and execution — but the SDK removed a lot of the manual work. JSON parsing disappeared, tool handling became cleaner and reliability improved. I also realized that building the raw version first helped me understand what the SDK was actually abstracting away.

One of the biggest lessons this week was that infrastructure reliability and reasoning reliability are not the same thing. The agent could make clean tool calls and return correct response formats while still making wrong assumptions. I saw this with the distance issue where the model assumed the distance between locations and produced an incorrect towing price. Nothing was broken technically — the problem was reasoning. CLAUDE.md was another lesson because I started seeing it as architecture rather than documentation since it defines the rules and behavior of the agent.

I also added a research sub-agent with isolation and file-based memory. Isolation showed me how heavy work can happen separately without flooding the main conversation with unnecessary context, while memory allowed the agent to remember conversations even after restarting. My biggest realization from Week 2 is that I started thinking agent engineering was mainly about models, but now I think much more about systems: Inputs → Tools → Workflow → Rules → Output. Week 1 taught me how to build agents, but Week 2 taught me how to think about them.
